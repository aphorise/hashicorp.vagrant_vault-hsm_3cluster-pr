...
iCLUSTERC_C = 0  # // Consul B INSTANCES UP TO 9 <= iN > 2
...
bCLUSTERC_CONSUL = false  # // Consul C use Consul as store for vault?

CLUSTERA_VAULT_NAME = 'hsm1'  # // Vault A Cluster Name
CLUSTERB_VAULT_NAME = 'hsm2'  # // Vault B Cluster Name
CLUSTERC_VAULT_NAME = 'hsm3'  # // Vault C Cluster Name
...
sCLUSTERC_IPS=''  # // Consul C - IPs constructed based on IP D class + instance number
...
su -l #{sVUSER} -c \"rsync -qva --rsh='ssh -i #{sHOME}/.ssh/id_rsa2' #{sVUSER}@#{sCLUSTERA_IP_CA_NODE}:~/vault#{iX}* :~/vault_init.json #{sHOME}/.\"
SCRIPT
				vault_node.vm.provision "shell", inline: $script
			end
...
#			vault_node.vm.provision "shell", inline: "/bin/bash -c '#{sHOME}/install_tls_ca_certs.sh #{iX == 1 ? iCLUSTERA_N : '' }'"
			...
			if iX == 1 then
				...
				vault_node.vm.provision "file", source: "#{sPTH}/6.post_setup_vault_leader_pr_enable.sh", destination: "#{sHOME}/post_setup_vault.sh"
...
su -l #{sVUSER} -c \"rsync -qva --rsh='ssh -i #{sHOME}/.ssh/id_rsa2' #{sVUSER}@#{sCLUSTERB_IP_CA_NODE}:~/vault#{iX}*  :~/vault_init.json #{sHOME}/.\"
SCRIPT
				vault_node.vm.provision "shell", inline: $script
			end
...
#			vault_node.vm.provision "shell", inline: "/bin/bash -c '#{sHOME}/install_tls_ca_certs.sh #{iX == 1 ? iCLUSTERB_N : '' }'"
			...
			if iX == 1 then
				# // DR specific script invoked by Vault Setup script.
				vault_node.vm.provision "file", source: "#{sPTH}/7.post_setup_vault_prefs.sh", destination: "#{sHOME}/post_setup_vault.sh"
	...
	# // ------ CLUSTER C ------ CLUSTER C ------
	# // Consul Server Nodes
	if bCLUSTERC_CONSUL then
		if iCLUSTERC_C == 0 then STDERR.puts "\e[31m#{sERROR_MSG_CONSUL}\e[0m" ; exit(3) ; end ;
		(1..iCLUSTERC_C).each do |iY|  # // CONSUL Server Nodes IP's for join (concatenation)
			sCLUSTERC_IPS+="\"#{sCLUSTERC_IP_CLASS_D}.#{iCLUSTERC_IP_CONSUL_CLASS_D+iY}\"" + (iY < iCLUSTERC_C ? ", " : "")
		end
		# // CONSUL AGENT SCRIPTS to setup
		config.vm.provision "file", source: "#{sPTH}/3.install_consul.sh", destination: "#{sHOME}/install_consul.sh"
		# // CONSUL Server Nodes
		(1..iCLUSTERC_C).each do |iY|
			config.vm.define vm_name="#{CLUSTERC_HOSTNAME_PREFIX}consul#{iY}" do |consul_node|
				consul_node.vm.hostname = vm_name
				consul_node.vm.network "public_network", bridge: "#{sNET}", ip: "#{sCLUSTERC_IP_CLASS_D}.#{iCLUSTERC_IP_CONSUL_CLASS_D+iY}"
				# consul_node.vm.network "forwarded_port", guest: 80, host: "5918#{iY}", id: "#{vm_name}"
				$script = <<-SCRIPT
sed -i 's/\"__IPS-SET__\"/#{sCLUSTERC_IPS}/g' #{sHOME}/install_consul.sh
/bin/bash -c #{sHOME}/install_consul.sh
SCRIPT
				consul_node.vm.provision "shell", inline: $script
			end
	...
	# // VAULT Server Nodes as Consul Clients as well.
	(1..iCLUSTERC_N).each do |iX|
		config.vm.define vm_name="#{CLUSTERC_HOSTNAME_PREFIX}vault#{iX}" do |vault_node|
			vault_node.vm.hostname = vm_name
			vault_node.vm.network "public_network", bridge: "#{sNET}", ip: "#{sCLUSTERC_IP_CLASS_D}.#{iCLUSTERC_IP_VAULT_CLASS_D-iX}"
			# vault_node.vm.network "forwarded_port", guest: 80, host: "5928#{iX}", id: "#{vm_name}"
...
su -l #{sVUSER} -c \"rsync -qva --rsh='ssh -i #{sHOME}/.ssh/id_rsa2' #{sVUSER}@#{sCLUSTERC_IP_CA_NODE}:~/vault#{iX}*  :~/vault_init.json #{sHOME}/.\"
SCRIPT
				vault_node.vm.provision "shell", inline: $script
			end
...
#			vault_node.vm.provision "shell", inline: "/bin/bash -c '#{sHOME}/install_tls_ca_certs.sh #{iX == 1 ? iCLUSTERC_N : '' }'"
			...
			# // ORDERED: copy VAULT TOKEN to .bashrc for convenience from main node after setup.
			if iX > 1 then
				vault_node.vm.provision "shell", inline: "su -l #{sVUSER} -c 'VT=$(ssh -i #{sHOME}/.ssh/id_rsa2 #{sVUSER}@#{sCLUSTERC_sIP_VAULT_LEADER} \"[[ -f /home/vagrant/vault_token.txt ]] && cat /home/vagrant/vault_token.txt || printf \'\'\"); if ! [[ ${VT} == \"\" ]] && ! grep VAULT_TOKEN ~/.bashrc ; then printf \"export VAULT_TOKEN=${VT}\n\" >> ~/.bashrc ; fi ;'"
			end
			...
			if iX == 1 then
				# // DR specific script invoked by Vault Setup script.
				vault_node.vm.provision "file", source: "#{sPTH}/7.post_setup_vault_prefs.sh", destination: "#{sHOME}/post_setup_vault.sh"
				vault_node.vm.provision "shell", inline: "/bin/bash -c '#{VV3} VAULT_CLUSTER_NAME='#{CLUSTERC_VAULT_NAME}' USER='#{sVUSER}' #{sHOME}/install_vault.sh'"
				# // OVER-WRITE: VAULT TOKEN to .bashrc from Cluster-A
				vault_node.vm.provision "shell", inline: "su -l #{sVUSER} -c 'VT=$(ssh -i #{sHOME}/.ssh/id_rsa1 #{sVUSER}@#{sCLUSTERA_sIP_VAULT_LEADER} \"[[ -f /home/vagrant/vault_token.txt ]] && cat /home/vagrant/vault_token.txt || printf \'\'\"); if ! [[ ${VT} == \"\" ]] && ! grep VAULT_TOKEN ~/.bashrc ; then printf \"export VAULT_TOKEN=${VT}\n\" >> ~/.bashrc ; fi ;'"
			else
				vault_node.vm.provision "shell", inline: "/bin/bash -c '#{VV3} #{VR3} VAULT_CLUSTER_NAME='#{CLUSTERC_VAULT_NAME}' USER='#{sVUSER}' #{sHOME}/install_vault.sh'"
			end

			# // DESTROY ACTION - need to perform raft peer remove if its not the last node:
			vault_node.trigger.before :destroy do |trigger|
				if iCLUSTERC_C == 0 && iCLUSTERC_N > 1 then
					trigger.run_remote = {inline: "printf 'RAFT CHECKING: if Removal from Qourum peers-list is required.\n' && bash -c 'set +eu ; export VAULT_TOKEN=\"$(grep -F VAULT_TOKEN #{sHOME}/.bashrc | cut -d= -f2)\" ; if (($(vault operator raft list-peers -format=json 2>/dev/null | jq -r \".data.config.servers|length\") == 1)) ; then echo \"RAFT: Last Node - NOT REMOVING.\" && exit 0 ; fi ; VS=$(vault status | grep -iE \"Raft\") ; if [[ \${VS} == *\"Raft\"* ]] ; then vault operator raft remove-peer \$(hostname) 2>&1>/dev/null && printf \"Peer removed successfully!\n\" ; fi ;'"}
				end
			end
		end
